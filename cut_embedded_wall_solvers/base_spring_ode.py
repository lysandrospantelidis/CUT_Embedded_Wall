...
def solve_base_spring_differential_equation(model: ModelInput) -> SolverResult:
    """Base-springs displacement-coupled CUT pressure solver with spring-release continuation.

    The 5th solver uses bilateral base springs only as a controlled regularisation path.
    ...
    spring_factors = [release_factor ** i for i in range(n_release_levels)]
    ...
    def _run_level(w_initial, spring_factor: float, level_index: int):
        ...
        try:
            top_deflection_abs = abs(float(w[0]))
            base_deflection_abs = abs(float(w[-1]))
        except Exception:
            top_deflection_abs = 0.0
            base_deflection_abs = 0.0

        base_dominated_shape = (
            base_deflection_abs > 1.20 * max(top_deflection_abs, 1.0e-12)
            and base_deflection_abs > 1.0e-4
        )
        ...
        # Preliminary acceptance at level-iteration scale.
        # Final spring-influence acceptance is checked after residual spring reactions
        # are evaluated for the accepted candidate.
        accepted_level = (
    (converged or engineering_converged)
    and not runaway
)
        ...
        return {
            ...
            'top_deflection_abs_m': top_deflection_abs,
            'base_deflection_abs_m': base_deflection_abs,
            'base_dominated_shape': base_dominated_shape,
    ...
    for level_index, factor in enumerate(spring_factors, start=1):
        ...
        print(
            ...
            "top_mm=", 1000.0 * float(level.get("top_deflection_abs_m", 0.0)),
            "base_mm=", 1000.0 * float(level.get("base_deflection_abs_m", 0.0)),
            "max_mm=", 1000.0 * float(level.get("max_deflection_abs_m", 0.0)),
        ...
        release_history.append({
            ...
            'top_deflection_abs_m': float(level.get('top_deflection_abs_m', 0.0)),
            'base_deflection_abs_m': float(level.get('base_deflection_abs_m', 0.0)),
            'base_dominated_shape': bool(level.get('base_dominated_shape', False)),
        })
        if level['accepted']:
    ...
    spring_dominated = spring_influence_ratio > 0.25
    if spring_influence_ratio < 0.05:
        spring_influence_class = 'weakly spring-supported'
    elif spring_influence_ratio <= 0.20:
        spring_influence_class = 'spring-influenced'
    else:
        spring_influence_class = 'artificially spring-supported'
    ...
    if engineering_converged and deflection_ratio >= 0.02:
        ...
        response_class = 'engineering-converged springs-release response'
        ...
        response_class = 'mechanism-like / unstable very-flexible base-springs release response'
        diagnostic_warning = (
            'The base-spring release could not find a fully admissible lower-restraint state. '
            + spring_release_msg
        )
    elif deflection_ratio >= 0.02:
        response_class = 'large-displacement springs-release response'
        diagnostic_warning = 'Large displacement relative to retained height. ' + spring_release_msg
    elif oscillation_count >= 3:
        response_class = 'oscillation-controlled springs-release response'
        diagnostic_warning = 'Adaptive relaxation detected repeated oscillation. ' + spring_release_msg
    else:
        response_class = 'stable springs-release base response'
        diagnostic_warning = spring_release_msg

    if spring_dominated:
        status = 'spring_dominated'
        msg = (
            f'Base-springs release solver stopped with excessive residual base-spring influence; '
            f'spring influence ratio={spring_influence_ratio:.3g}. {spring_release_msg}.'
        )

    elif converged:
        status = 'ok'
        msg = f'Base-springs release solver completed; {spring_release_msg}.'

    elif engineering_converged:
        status = 'engineering_converged'
        msg = (
            f'Base-springs release solver accepted as engineering-converged; '
            f'{engineering_reason}. {spring_release_msg}.'
        )

    else:
        status = 'not_converged'
        msg = (
            f'Base-springs release solver did not converge; '
            f'max Δw={last_diff:.3e}. {spring_release_msg}.'
        )
    ...
    return SolverResult(
        ...
        summary={
            'solver': 'Base springs (differential equation) — spring-release continuation',
            ...
            'n_release_levels': n_release_levels,
            ...
            'spring_dominated': spring_dominated,
            'spring_influence_percent': 100.0 * spring_influence_ratio,
            'spring_influence_classification': spring_influence_class,
            'spring_influence_rule': '<5% weakly spring-supported; 5–20% spring-influenced; >20% artificially spring-supported',
            'relaxation_factor_final': final_relax,
            'relaxation_factor_bounds': (0.03, 0.35),
            'stabilization': 'adaptive under-relaxation plus progressive base-springs release continuation; springs are reduced and only the lowest accepted restraint level is reported',
            'EI': EI,
            ...
            'top_deflection_abs_m': float(accepted.get('top_deflection_abs_m', 0.0)),
            'base_deflection_abs_m': float(accepted.get('base_deflection_abs_m', 0.0)),
            'base_dominated_shape': bool(accepted.get('base_dominated_shape', False)),
            'shape_admissibility_rule': '|w_base| must not exceed 1.20·|w_top| when base displacement is significant',
        },
    )
