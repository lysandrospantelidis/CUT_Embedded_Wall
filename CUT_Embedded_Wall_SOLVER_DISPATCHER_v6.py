...
def solve(model: ModelInput, progress_callback=None) -> SolverResult:
    ...
    if getattr(model, "reinforcement_supports", None) and mode not in {"fixed_base_differential_equation", "base_spring_differential_equation"}:
        raise ValueError(
            "Reinforcement/supports require a differential-equation solver with nodal support formulation. "
            "Closed-form, rigid-wall and general-case solvers are disabled for reinforcement until their nonlinear support formulation is implemented."
        )
