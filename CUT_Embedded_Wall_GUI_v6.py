...
PROGRAM_VERSION = "CUT Embedded Wall v9.1.1"
...
class App(tk.Tk):
    ...
    def _collect_project_data(self):
        return {
            ...
            "inputs": {
                ...
                "variational_base_condition": self.var_variational_base_condition.get(),
                "variational_base_spring_factor": self.var_variational_base_spring_factor.get(),
                "variational_spring_release_factor": self.var_variational_spring_release_factor.get(),
    ...
    def _apply_project_data(self, data):
        ...
        setters = {
            ...
            "variational_base_condition": self.var_variational_base_condition,
            "variational_base_spring_factor": self.var_variational_base_spring_factor,
            "variational_spring_release_factor": self.var_variational_spring_release_factor,
    ...
    def _build_vars(self):
        ...
        self.var_variational_base_condition = tk.StringVar(value="Free-like")
        self.var_variational_base_spring_factor = tk.DoubleVar(value=1.0e-4)
        self.var_variational_spring_release_factor = tk.DoubleVar(value=0.80)
    ...
    def _load_engineering_icons(self):
        ...
        def load_scaled_icon(key, max_px=172):
            path = os.path.join(icons_dir, f"{key}.png")
            try:
                img = tk.PhotoImage(file=path)
                max_dim = max(img.width(), img.height())
                if max_dim > max_px:
                    factor = max(1, math.ceil(max_dim / max_px))
        ...
        for key in (
            "fixed_closed", "fixed_diff", "rigid", "general", "general_variational",
            "none", "anchor", "prop", "geogrid", "strip", "metalgrid",
        ):
            self._eng_icons[key] = load_scaled_icon(key)
    ...
    def _make_icon_button(self, parent, icon_key, title, subtitle, command, bg="#ffffff", fg="#172033", width=None, height=None):
        ...
        try:
            btn.configure(width=min(img.width(), 172), height=min(img.height(), 172))
        except Exception:
            pass
    ...
    def _reinforcement_solver_lock_message(self) -> str:
        return "Reinforcement active → differential-equation or general variational solver required."
    ...
    def _build_run_tab(self):
        ...
        # Icon-only solver selector. Base spring is intentionally removed from this branch;
        # reinforcement-compatible nonlinear runs are handled by differential equation
        # and the general variational formulation.
        ...
        card_specs = [
            ...
            ("general_variational", "#eef2ff", "#3730a3", "Any wall (general variational)"),
        ]
        ...
        self.variational_box = ttk.LabelFrame(panel, text="General variational controls")
        variational_box = self.variational_box
        variational_box.grid(row=5, column=0, sticky="ew", pady=5)
        ttk.Label(variational_box, text="Base condition").grid(row=0, column=0, sticky="w", padx=4, pady=3)
        ttk.OptionMenu(
            variational_box,
            self.var_variational_base_condition,
            self.var_variational_base_condition.get(),
            "Free-like", "Weak spring", "Restrained-like",
        ).grid(row=0, column=1, sticky="ew", padx=4, pady=3)
        self._entry(variational_box, 1, 0, "Variational base spring factor", self.var_variational_base_spring_factor)
        self._entry(variational_box, 2, 0, "Variational spring release factor", self.var_variational_spring_release_factor)
    ...
    def validate_current_model(self, result=None):
        ...
        if result is not None:
            ...
            if diag_warning:
                add("WARNING", "Solver diagnostic", diag_warning)
            try:
    ...
    def _select_solver_display(self, display_name: str):
        # With reinforcement active, allow both supported nonlinear solvers:
        # fixed-base differential and general variational.  Do not force every
        # reinforcement click back to the fixed-base solver when an already
        # compatible variational solver is selected.
    ...
    def _update_solver_visibility(self):
        ...
        try:
            ...
            if hasattr(self, "variational_box"):
                if "general variational" in name.lower():
                    self.variational_box.grid()
                else:
                    self.variational_box.grid_remove()
        except Exception:
            pass
    ...
    def build_model_input(self):
        ...
        # so the GUI remains compatible with older SolverControls dataclasses.
        controls.variational_base_condition = str(self.var_variational_base_condition.get())
        controls.variational_base_spring_factor = max(1.0e-8, float(self.var_variational_base_spring_factor.get()))
        controls.variational_spring_release_factor = max(0.20, min(0.98, float(self.var_variational_spring_release_factor.get())))
