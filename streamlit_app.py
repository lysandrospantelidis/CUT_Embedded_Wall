...
SOLVER_CARDS = [
    ...
    ("general_variational", "Any wall (general variational)", "General variational"),
]
...
def init_state() -> None:
    ...
    "stage_q_R_apply": "Stage 0",
        ...
        "variational_base_condition": "Free-like", "variational_base_spring_factor": 1.0e-4, "variational_spring_release_factor": 0.80,
        "query_z": 2.0,
...
def handle_selector_query_params() -> None:
    ...
    for state_key, target_page, cards in selector_specs:
        ...
        if selected in valid_values:
            ...
            # Keep the selected solver whenever it is compatible with reinforcement.
            # Older Streamlit builds forced the differential solver every time a
            # reinforcement card was clicked. That erased the new variational beam
            # selection and could also make the UI fall back to No reinforcement on
            # the next icon navigation.
            if state_key == "reinforcement_type" and selected != "No reinforcement":
                if st.session_state.get("solver_display") not in REINFORCEMENT_ALLOWED_SOLVERS:
...
def enforce_solver_rule():
    if st.session_state.reinforcement_type != "No reinforcement":
        if st.session_state.solver_display not in REINFORCEMENT_ALLOWED_SOLVERS:
            st.session_state.solver_display = REINFORCEMENT_REQUIRED_SOLVER
            st.warning("Solver automatically switched to a reinforcement-compatible method.")
...
def build_model() -> Any:
    ...
    # Variational-beam base-condition controls are attached after construction.
    # This keeps the Streamlit app compatible even when an older SolverControls
    # dataclass is imported from cache/deployment during a hot reload.
    controls.variational_base_condition = str(st.session_state.get("variational_base_condition", "Free-like"))
    controls.variational_base_spring_factor = max(1.0e-8, float(st.session_state.get("variational_base_spring_factor", 1.0e-4)))
    controls.variational_spring_release_factor = max(0.20, min(0.98, float(st.session_state.get("variational_spring_release_factor", 0.80))))
    if H_L <= 0 or H_R <= 0:
...
def image_selector(cards: list[tuple[str, str, str]], state_key: str, target_page: str, columns: int) -> None:
    ...
    - solver: 3 cards on the first row and 3 cards on the second row
    """
    query_key = f"select_{state_key}"
    ...
    for icon, value, label in cards:
        ...
        # Solver-card clicks must preserve the active reinforcement choice.
        # Otherwise the URL navigation can rerun the app with only the solver
        # parameter visible and Streamlit may show the full unreinforced solver grid.
        if state_key == "solver_display":
            active_reinf = st.session_state.get("reinforcement_type", "No reinforcement")
            href = f"?select_reinforcement_type={quote(active_reinf)}&{query_key}={quote(value)}"
        else:
...
/* Previous */
button[kind="secondary"]:has(div:contains("Previous")),
button[kind="secondary"]:has(p:contains("Previous")){
...
/* Next */
button[kind="secondary"]:has(div:contains("Next")),
...
""", unsafe_allow_html=True)
    ...
    if st.session_state.reinforcement_type != "No reinforcement":
        st.markdown(
            '<div class="cut-warning">Reinforcement is active; only reinforcement-compatible solvers are available: Differential equation, and General variational.</div>',
            unsafe_allow_html=True
        )
...
def render_run():
    ...
    numerical_row3 = pd.DataFrame({
        ...
        "Variational spring release factor": [_cut_fmt(st.session_state.get("variational_spring_release_factor", 0.80), "{:.2f}")],
    })
    edited_num3 = st.data_editor(
        ...
        num_rows="fixed",
        height=80,
        key="numerical_controls_compact_row3_v2_release_factor_visible",
        column_config={
            ...
            "Variational spring release factor": st.column_config.TextColumn(
                "Variational spring release factor",
                width="medium",
                help=(
                    "Used by the General variational solver. At each continuation level the base translational "
                    "and rotational springs are multiplied by this factor. Smaller values such as 0.50 release the base "
                    "more aggressively and may create larger jumps; larger values such as 0.80–0.90 give a smoother, "
                    "more stable continuation path but require more levels and more computation time."
                ),
            ),
        },
    )
    try:
        ...
        st.session_state.variational_spring_release_factor = max(0.20, min(0.98, _cut_float(edited_num3.loc[0, "Variational spring release factor"], 0.80)))
    except Exception:
        pass

    if st.session_state.get("solver_display") == "Any wall (general variational)":
        st.info(
            "Variational spring release factor: controls the continuation step for the General variational solver. "
...
def render_stages_and_reinforcement(model_preview: Any):
    ...
    # v7.7: keep a stable two-pane layout for both No reinforcement and
    # active reinforcement.  Previously, selecting a reinforcement type replaced
    # the columns with sequential containers, so the reinforcement selector and
    # preview moved below the excavation-stage pane.
...
def render_advanced():
    ...
    st.markdown("#### General case variational beam — base restraint")
    c1, c2 = st.columns(2, gap="small")
    c1.selectbox(
        "Variational base condition",
        options=["Free-like", "Elastic base", "Fixed-like", "Custom"],
        key="variational_base_condition",
        help="Controls the explicit base springs of the general-case variational beam solver. Default Free-like uses a weak rotational base restraint only; translational base spring is zero.",
    )
    c2.number_input(
        "Custom base spring factor",
        key="variational_base_spring_factor",
        min_value=1.0e-8,
        max_value=1.0e3,
        format="%.6g",
        help="Used only when Variational base condition = Custom. It defines the minimum target spring factor for the continuation path. Both translational and rotational base springs are progressively reduced toward this value unless an instability or physical-shape stop is detected.",
    )
    st.number_input(
        "Variational spring release factor",
        key="variational_spring_release_factor",
        min_value=0.20,
        max_value=0.98,
        step=0.05,
        format="%.2f",
        help=(
            "Controls the continuation step used by the general-case variational beam solver. "
            "At each release level the base translational and rotational springs are multiplied by this factor. "
            "Smaller values, e.g. 0.50, release the base quickly and may reach mechanism behaviour faster but with larger jumps. "
            "Larger values, e.g. 0.80–0.90, produce a smoother and usually more stable continuation path, but require more release levels and more computation time. "
            "The solver still applies adaptive physical-shape and instability checks and reports the last accepted admissible release level."
        ),
    )
    result = st.session_state.last_result
    if result is not None:
...
@media(max-width:900px){
    ...
    .img-grid.solver{grid-template-columns:repeat(3,minmax(0,1fr))!important;}
